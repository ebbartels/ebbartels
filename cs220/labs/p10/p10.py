...
for star in stars_1_dict:
    val = stars_1_dict[star].stellar_luminosity
    if not val == None:
        avg_lum_stars_1 += val
        total +=1

avg_lum_stars_1 = avg_lum_stars_1/total
avg_lum_stars_1
# -
grader.check("q7")
...
for star in stars_2_dict:
    val = stars_2_dict[star].stellar_age
    if not val == None:
        avg_age_stars_2 +=val
        total +=1
avg_age_stars_2 = avg_age_stars_2/total
avg_age_stars_2
...
biggest_star =None
largest_stellar_radius = 0
for star in stars_dict:
    val = stars_dict[star].stellar_radius
    if val == None:
        continue
    elif biggest_star==None or val>largest_stellar_radius:
        largest_stellar_radius=val
        biggest_star=star
biggest_star
...
for star in stars_dict:
    if star.startswith('Kepler'):
        if stars_dict[star].stellar_age==None:
            ...
            avg_age_kepler+=stars_dict[star].stellar_age
            total+=1
avg_age_kepler=avg_age_kepler/total
avg_age_kepler
...
def planet_cell(row_idx, col_name, planets_rows, header=planets_header):
    col_idx = planets_header.index(col_name) 
...
for idx in range(len(planets_2)):
    if planets_2[idx].controversial_flag == True:
        controversial_planets.append(planets_2[idx])
controversial_planets
grader.check("q14")
...
for file in files_in_data:
    if file.startswith('planets'):
        planet_file_paths.append(os.path.join('data',file))
    
planet_file_paths.sort()
...
planets_list = []
for i in range(5):
    json_i = json_file_paths[i]
    csv_i = planet_file_paths[i]
...
for i in range(len(planets_list)):
    if planets_list[i].discovery_year == 2022:
        planets_disc_2022+=1
...
toi_2022_c_star_name = None
for i in range(len(planets_list)):
    if planets_list[i].planet_name == 'TOI-2202 c':
        toi_2022_c_star_name = planets_list[i].host_name
        break
toi_2022_c_star = stars_dict[toi_2022_c_star_name]
toi_2022_c_star
grader.check("q18")
...
for i in range(len(planets_list)):
    val2 = planets_list[i].planet_radius
    if val2 == None:
        continue
    elif planets_list[i].host_name in star_list:
        avg_planet_radius_big_stars += val2
        total+=1
avg_planet_radius_big_stars=avg_planet_radius_big_stars/total
...
for star in stars_dict:
    val = stars_dict[star].stellar_age
    name = star
    if val == None:
        continue
    elif youngest_star_name==None or val < youngest_star:
        youngest_star = val
        youngest_star_name = name
for i in range(len(planets_list)):
    if planets_list[i].host_name == youngest_star_name:
        youngest_star_planets.append(planets_list[i])
youngest_star_planets
grader.check("q20")
